        ...
        <aside class="eap-sidebar">
            ...
            <nav class="eap-sidebar__nav">
                <a href="<?= htmlspecialchars(($admin_base_path ?? '/admin') . '/dashboard') ?>" class="eap-sidebar__link <?= ($page_title ?? '') === 'Dashboard' ? 'eap-sidebar__link--active' : '' ?>">
                    ...
                    <span class="eap-sidebar__link-text">operation system</span>
                </a>
                ...
                <?php if (!empty($tabs)) : ?>
                    ...
                    // Helper to build URL with admin base path
                    $buildTabUrl = function($url) use ($admin_base_path) {
                        if (str_starts_with($url, '/admin/')) {
                            return $admin_base_path . '/' . substr($url, 7);
                        } elseif ($url === '/admin') {
                            return $admin_base_path;
                        } elseif (str_starts_with($url, '/') && !str_starts_with($url, $admin_base_path)) {
                    ...
                    foreach ($tabs as $tab) :
                        ...
                        <?php endif; ?>
                    <?php
                        $tabUrl = $buildTabUrl($tab['url']);
                        $hasChildren = !empty($tab['children']);
                        $isParentActive = str_starts_with($currentPath, $tabUrl);
                    ?>

                    <?php if ($hasChildren) : ?>
                    <!-- Parent with children - render as expandable group -->
                    <div class="eap-sidebar__group <?= $isParentActive ? 'eap-sidebar__group--expanded' : '' ?>">
                        <a href="<?= htmlspecialchars($tabUrl) ?>" class="eap-sidebar__link <?= $isParentActive ? 'eap-sidebar__link--active' : '' ?>">
                            <svg class="eap-sidebar__link-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <?= $iconPaths[$tab['icon'] ?? 'default'] ?? $iconPaths['default'] ?>
                            </svg>
                        ...
                        <!-- Children submenu -->
                        <div class="eap-sidebar__submenu">
                            <?php foreach ($tab['children'] as $child) :
                                $childUrl = $buildTabUrl($child['url']);
                                $isChildActive = $currentPath === $childUrl ||
                                                (isset($page_title) && strtolower($page_title) === strtolower($child['label']));
                            ?>
                            <a href="<?= htmlspecialchars($childUrl) ?>" class="eap-sidebar__sublink <?= $isChildActive ? 'eap-sidebar__sublink--active' : '' ?>">
                                <?= htmlspecialchars(strtolower($child['label'])) ?>
                            </a>
                            <?php endforeach; ?>
                        </div>
                    </div>
                    ...
                    <?php
                        $isActive = $currentPath === $tabUrl ||
                                   (isset($page_title) && strtolower($page_title) === strtolower($tab['label']));
                    ?>
                    <a href="<?= htmlspecialchars($tabUrl) ?>" class="eap-sidebar__link <?= $isActive ? 'eap-sidebar__link--active' : '' ?>">
                        <svg class="eap-sidebar__link-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <?= $iconPaths[$tab['icon'] ?? 'default'] ?? $iconPaths['default'] ?>
                        </svg>
                        <span class="eap-sidebar__link-text"><?= htmlspecialchars(strtolower($tab['label'])) ?></span>
